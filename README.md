...
- **要約**: Google Gemini API (`gemini-2.5-flash`、無料枠) で日本語 2〜3 文に要約
...
python scripts/summarize.py     # Gemini で要約 -> docs/data/YYYY-MM-DD.json
...
### ステップ 1: Google Gemini API キーを発行 (完全無料)

Gemini API は無料枠 (1 日 1,500 リクエスト、毎分 15 リクエスト) の範囲で本サイトを動かせます。本サイトは 1 日 4 回しか呼ばないため余裕で収まります。クレジットカード登録も不要です。

1. Google アカウントでログインした状態で `https://aistudio.google.com/app/apikey` を開く
2. **"Create API key"** をクリック
3. プロジェクトを聞かれたら "Create API key in new project" を選ぶ
4. 表示された `AIza...` で始まる文字列を**その場でコピー**
   - 後から再表示も可能ですが、一旦コピーしておくと楽
5. (任意) "Set up Billing" は **押さない**。押すと有料プランに移行するリスクがあります

### ステップ 2: GitHub Secrets に登録

GitHub Actions の cron ジョブがこのキーを使って Gemini API を呼びます。コードに書くと漏洩するので必ず Secrets に登録します。
...
登録後、Secrets 一覧に `GEMINI_API_KEY` が出ていれば OK (値そのものは GitHub 上でも見えなくなります。これが正しい挙動です)。
...
| 症状 | 原因 | 対処 |
|------|------|------|
| `GEMINI_API_KEY is not set` | Secret 未登録 / 名前が違う | ステップ 2 を再確認 |
| `401 / 403 PERMISSION_DENIED` | キーが無効 / 権限なし | AI Studio で新キー発行 → Secret 更新 |
| `429 RESOURCE_EXHAUSTED` | 無料枠の上限超過 (通常起きない) | 翌日まで待つ、またはモデルを軽量版に変更 |
